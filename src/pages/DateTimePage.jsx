...
const DateTimePage = ({ finalResource, reservations, onBack, onLogout, onDateTimeSubmit, onLogoClick, userId }) => {
    ...
    const checkConflict = useCallback(() => {
        ...
        if (approvedConflict) {
            setConflict({ type: 'error', message: 'Este horário entra em conflito com uma reserva já APROVADA.' });
            return;
        }
        ...
        if (pendingConflict) {
            setConflict({ type: 'warning', message: 'Este horário já tem um pedido PENDENTE. Você não pode criar uma nova reserva, mas pode declarar sua intenção de uso abaixo.' });
            return;
        }
                        ...
                        <div className="fade-in">
                            ...
                            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mb-2">
                                <div className="w-full sm:w-1/2">
                                    ...
                                    <input type="time" id="start-time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className={`w-full p-3 border rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 ${conflict.type === 'error' ? 'border-red-500' : 'border-slate-300'}`} />
                                </div>
                                <div className="w-full sm:w-1/2">
                                    <label htmlFor="end-time" className="block text-sm font-medium text-slate-700 mb-1">Fim</label>
                                    <input type="time" id="end-time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className={`w-full p-3 border rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 ${conflict.type === 'error' ? 'border-red-500' : 'border-slate-300'}`} />
                                </div>
                            </div>
                            
                            {conflict.message && <p className={`text-sm mt-2 mb-6 ${conflict.type === 'error' ? 'text-red-600' : 'text-amber-600'}`}>{conflict.message}</p>}
                                ...
                                <div className="mt-6 flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                                    ...
                                    {conflict.type === 'warning' && (
                                        <button
                                            ...
                                            className="w-full bg-orange-500 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-orange-600 transition-all transform hover:scale-105"
                                        >
                            ...
                            {intentionDeclared && (
                                <div className="mt-6 p-4 bg-green-100 text-green-800 border border-green-300 rounded-lg text-center">
                                    ...
                                    <p>A sua intenção de uso para este horário foi registada. Isto ajuda a equipe de TI a medir a demanda por recursos. <strong>Isto NÃO é um pedido de reserva.</strong></p>
                                </div>
                            )}
                        </div>
                    )}
