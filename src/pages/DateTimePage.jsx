...
const DateTimePage = ({ finalResource, reservations, onBack, onLogout, onDateTimeSubmit, onLogoClick, userId }) => {
    const [date, setDate] = useState(null);
    const [startTime, setStartTime] = useState('');
    const [endTime, setEndTime] = useState('');
    const [conflict, setConflict] = useState({ type: null, message: '' });
    const [intentionDeclared, setIntentionDeclared] = useState(false);

    const checkConflict = useCallback(() => {
        setIntentionDeclared(false);
        ...
        const anyConflict = activeReservations.find(res => res.id_recurso_final === finalResource.id && timeOverlap(res));

        if (anyConflict) {
            setConflict({ type: 'warning', message: 'Este horário se encontra OCUPADO. Declare sua intenção de reserva para ajudar a equipe de TI a expandir os recursos no FUTURO.' });
            return;
        }
    ...
    const handleDeclareIntention = async () => {
        try {
            const response = await fetch('http://localhost:3001/api/intencao-reserva', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    usuarioId: userId,
                    recursoNome: finalResource.nome,
                    dataInicio: `${date}T${startTime}:00`,
                    dataFim: `${date}T${endTime}:00`
                })
            });
            if (!response.ok) throw new Error('Falha ao registar a intenção.');
            setIntentionDeclared(true);
        } catch (error) {
            console.error(error);
            alert('Ocorreu um erro ao registar a sua intenção. Por favor, tente novamente.');
        }
    };
    ...
    const isDateLocked = (day) => { const today = new Date(); today.setHours(0,0,0,0); if (day.getTime() < today.getTime()) return true; return false; };
    const handleSubmit = () => { if (conflict.type !== null) { alert(conflict.message); } else if (date && startTime && endTime) { onDateTimeSubmit({ date, startTime, endTime }); } else { alert('Por favor, selecione data e horários.'); } };
                        ...
                        <div className="fade-in">
                            ...
                            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mb-2">
                                <div className="w-full sm:w-1/2">
                                    ...
                                    <input type="time" id="start-time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className={`w-full p-3 border rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 ${conflict.type === 'warning' ? 'border-red-500' : 'border-slate-300'}`} />
                                </div>
                                <div className="w-full sm:w-1/2">
                                    <label htmlFor="end-time" className="block text-sm font-medium text-slate-700 mb-1">Fim</label>
                                    <input type="time" id="end-time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className={`w-full p-3 border rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 ${conflict.type === 'warning' ? 'border-red-500' : 'border-slate-300'}`} />
                                </div>
                            </div>
                            
                            {conflict.message && <p className={`text-sm mt-2 mb-6 font-semibold text-red-600`}>{conflict.message}</p>}

                            {startTime && endTime && !intentionDeclared && (
                                <div className="mt-6 flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                                    <button 
                                        onClick={handleSubmit} 
                                        disabled={conflict.type !== null}
                            ...
                            {intentionDeclared && (
                                <div className="mt-6 p-4 bg-red-100 text-red-800 border border-red-300 rounded-lg text-center">
                                    <h3 className="font-bold text-lg">Isto NÃO é um pedido de reserva.</h3>
                                    <p>Obrigado! A sua intenção de uso para este horário foi registrada. Isto ajuda a equipe de TI a medir a demanda por recursos.</p>
                                </div>
                            )}
                        </div>
                    )}
