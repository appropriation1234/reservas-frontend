    ...
    return (
        <div className="bg-slate-50 min-h-screen">
            ...
            <main className="w-full max-w-7xl mx-auto p-4 sm:p-8">
                ...
                {isLoading && <p>A carregar as suas reservas...</p>}
                {!isLoading && myReservations.length === 0 && <p>Você ainda não fez nenhum pedido de reserva.</p>}
                {!isLoading && myReservations.length > 0 && (
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left text-slate-500">
                            <thead className="text-xs text-slate-700 uppercase bg-slate-100"><tr><th scope="col" className="px-6 py-3">Recurso</th><th scope="col" className="px-6 py-3">Data & Hora</th><th scope="col" className="px-6 py-3">Status</th><th scope="col" className="px-6 py-3">Ações</th></tr></thead>
                            <tbody>
                                {myReservations.map(reserva => (
                                    <tr key={reserva.ReservaID} className="bg-white border-b hover:bg-slate-50">
                                        <td className="px-6 py-4 font-semibold text-slate-900">{reserva.NomeSubRecurso || reserva.NomeRecursoPai}</td>
                                        ...
                                        <td className="px-6 py-4">{canCancel(reserva) && (<button onClick={() => handleCancel(reserva.ReservaID)} className="font-medium text-red-600 hover:underline">Cancelar</button>)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
